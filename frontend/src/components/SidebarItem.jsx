...
export default function SidebarItem({ icon, label, path, active, onClick, action }) {
  const navigate = useNavigate();

  const handleClick = () => {
    if (action === "logout") {
      // Hapus data session/login kalau ada
    ...
    // Jika ada fungsi custom (misalnya onClick)
    if (onClick) {
      onClick();
      return;
    }

    // Jika ada path, navigasikan ke situ
    if (path) {
      navigate(path);
    }
  };
